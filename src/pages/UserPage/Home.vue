<template>
  <div class="min-h-screen min-w-screen flex flex-col bg-[#F9FAFB] dark:bg-gray-900 font-poppins dark:text-gray-100">
    ...
    <!-- CombineCharts component now uses Plotly.js instead of Chart.js -->
    <CombineCharts
...
// ONLY CHANGE: CombineCharts component - now migrated to Plotly.js (no more Chart.js dependency)
...
/**
 * Estimated Savings const for SourcesChart.vue to display the estimated savings.
 * 
 * The savings are just the amount of kWh generated by solar × how much 
 * it would've cost if you bought that kWh from the grid (VECO rate). */ 

const estimatedSavings = computed(() => {
  return solarKwh.value * currentRate.value;
});

// Pesos Formatter to display appropriate PH Pesos money format
...
/***
 * Fetches the latest Carbon Emissions equivalent per kwh in Grid. 
 * Average equivalent of carbon emissions per kwh in the Philippines
 * is 0.7122 kg. 
 */
...
onMounted(async () => {
  ...
  fetchUtilityRate(); // Fetch utility rate of Pesos per KWH from VECO
  fetchCarbonRate(); // Fetch Carbon Emission kg equivalent per kwh
