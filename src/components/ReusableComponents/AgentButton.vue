<template>
  <div class="fixed bottom-6 right-6 z-50 font-poppins">
    ...
    <transition name="fade">
      <div v-if="isVoiceMode" class="fixed inset-0 z-[100] flex flex-col items-center justify-between bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-all duration-500 p-6 md:p-12">
        ...
        <!-- MAIN LIQUID ORB (Center - Clickable) -->
        <div class="relative flex-1 flex items-center justify-center w-full cursor-pointer"
             @click="toggleRecording">
             
             <!-- The Blob Animation -->
             <div class="blob relative w-64 h-64 md:w-80 md:h-80 flex items-center justify-center transition-all duration-500"
                  :class="{
                ...
                <!-- Gradient Layers -->
                <div class="absolute inset-0 bg-gradient-to-tr from-emerald-400 to-green-600 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob"></div>
                <div class="absolute inset-0 bg-gradient-to-bl from-teal-300 to-emerald-500 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-2000"></div>
                <div class="absolute inset-0 bg-gradient-to-r from-green-300 to-emerald-400 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-4000"></div>
             </div>

             <!-- Status Text -->
             <div class="absolute mt-80 md:mt-96 text-center pointer-events-none transition-opacity duration-300" :class="{'opacity-50': isProcessing}">
        ...
        <div class="w-full flex justify-between items-center px-4 md:px-8 pb-4 md:pb-8">
           ...
           <button 
             ...
             class="p-5 rounded-full transition-all duration-300 shadow-sm hover:shadow-md focus:outline-none"
             :class="[
                isRecording 
...
/* Abstract Liquid Blob Animation */
@keyframes blob {
  0% {
